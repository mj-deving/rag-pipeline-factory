...
Measured across 4 successful factory runs vs a designed traditional n8n agent approach. Full methodology and data in [`benchmark.md`](benchmark.md).

| Metric | Traditional | Factory | Savings |
|---|---|---|---|
| n8n nodes | 9-16 | **5** | 44-69% fewer |
| LLM rounds per build | 14 | **8.25** | 41% fewer |
| Total tokens per build | ~131K | **~58K** | **56% savings** |
| LLM cost per build | $0.122 | **$0.052** | 57% cheaper |
| Avg execution time | N/A | **74s** | - |
| Template generation | LLM-generated | **20ms deterministic** | 100% reliable |

The key advantage: workflow JSON is produced by a deterministic template in 20ms instead of being generated by the LLM (~3,000 output tokens eliminated per build).
